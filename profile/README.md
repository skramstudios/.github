...
| [skram](https://github.com/skramstudios/skram) | One job queue for the things your agents cannot each have a copy of: the Docker VM, the local Kubernetes cluster, the homelab host. One job at a time per resource, with who asked, a human `hold`, `wait`, and `explain`. CLI + MCP server. |
| [skram-tunnel](https://github.com/skramstudios/skram-tunnel) | Share a local app *and the identity provider it logs in against* on one public URL, so a reviewer on a phone can get past the login page. |
