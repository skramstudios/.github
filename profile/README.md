...
| [skram](https://github.com/skramstudios/skram) | One job queue for the things your agents cannot each have a copy of: the Docker VM, the local Kubernetes cluster, a shared build host. One job at a time per resource, with who asked, a human `hold`, `wait`, and `explain`. CLI + MCP server. |
